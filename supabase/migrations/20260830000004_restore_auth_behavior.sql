-- Migration: Restore Production Auth Behavior for Transfer Credits
-- Created At: 2026-08-30

-- 1. Redefine find_recipient_by_phone to match the existing production security model
...
BEGIN
  ...
  -- Resolve sender: use the existing production security model (JWT sub claim or x-user-id header fallback)
  ...
  -- Match recipient using last 10 digits of phone in public.users (allowing customer or trainer roles)
...
-- 2. Redefine transfer_credits to match the existing production security model
...
-- 3. Redefine SELECT policy on credit_lots to match the existing production security model
...
-- 4. Redefine INSERT policy on invitations to match the existing production security model
