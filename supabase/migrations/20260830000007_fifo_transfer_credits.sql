-- Migration: FIFO Credit Lot Transfer and Allocation
-- Created At: 2026-08-30

-- 1. Redefine transfer_credits to support FIFO multi-lot splitting and return full allocation breakdowns
...
BEGIN
  ...
  -- FIFO allocation: consume from oldest active credit lot first
  v_needed := p_amount;
  FOR v_lot IN 
  ...
  LOOP
    ...
    IF v_lot.remaining_credits >= v_needed THEN
      ...
      INSERT INTO public.credit_lots (
        ...
        v_needed, v_needed, v_orig_lot_id
      );

      v_allocations := v_allocations || jsonb_build_object(
        'amount', v_needed,
        'original_lot_id', v_orig_lot_id,
        'original_purchase_date', to_char(v_lot.original_purchase_date, 'YYYY-MM-DD'),
        'official_expiry_date', to_char(v_lot.official_expiry_date, 'YYYY-MM-DD')
      );
    ...
    ELSE
      ...
      INSERT INTO public.credit_lots (
        ...
        v_to_client_id, v_lot.original_purchaser_id, v_lot.original_purchase_date, 
        v_lot.official_expiry_date, v_lot.grace_expiry_date, 
        v_transferred, v_transferred, v_orig_lot_id
      );

      v_allocations := v_allocations || jsonb_build_object(
        'amount', v_transferred,
        'original_lot_id', v_orig_lot_id,
        'original_purchase_date', to_char(v_lot.original_purchase_date, 'YYYY-MM-DD'),
        'official_expiry_date', to_char(v_lot.official_expiry_date, 'YYYY-MM-DD')
      );
