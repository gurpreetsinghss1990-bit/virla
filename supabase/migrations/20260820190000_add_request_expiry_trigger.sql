-- Migration: Add Request Expiry validation to timeline_status transition trigger
...
BEGIN
  ...
  ELSIF NEW.timeline_status = 'trainer_accepted' THEN
    ...
    -- PROBLEM 2: Authoritative server/database check for request expiry
    -- Reject acceptance if request is older than 60 seconds (60000 ms) from creation
    IF now_ms - OLD.created_at >= 60000 THEN
      RAISE EXCEPTION 'Request has expired. The 60-second acceptance window has closed.';
    END IF;
